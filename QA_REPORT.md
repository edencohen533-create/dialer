...
## סבב המשך — תקלות תזמון בהאזנת מנהל

בעקבות בקשת המשך נמצאו ושוחזרו שלוש תקלות נוספות. כולן נכשלו בבדיקה לפני תיקון ועברו אחריו, בספק מדומה ובמסד המקומי בלבד.

| חומרה | שחזור | תיקון | אימות |
|---|---|---|---|
| גבוהה | ביטול האזנה בזמן שבקשת dialSupervisor עדיין ממתינה; הספק מחזיר leg לאחר הביטול | שמירת מזהה החיבור המאוחר, בדיקת מצב ההאזנה והשיחה מחדש וניתוקו | F7 עבר |
| גבוהה | אירוע סיום האזנה מגיע לפני תשובת בקשת מעבר ללחישה | עדכון מצב מותנה בכך שהשיחה וההאזנה עדיין פעילות; אחרת 409, ללא דריסת ended | F8 עבר |
| גבוהה | conference.joined מגיע אחרי ביטול, כשמזהה החיבור טרם נשמר | שמירת leg מהאירוע וניתוק חיבור מאוחר; כשל ניתוק אינו מסמן את האירוע כמעובד ומאפשר replay | F9 עבר, כולל כשל ו־redelivery |

נוסף guard למעבר connecting→listening כדי שאירוע הצטרפות לא ידרוס סיום מקביל. במסלול התנגשות של שתי בקשות התחלת האזנה, מוחזרת האזנה קיימת רק אם היא לאותה שיחה.

אימות: תשע בדיקות fault injection עברו; U20 עבר שוב ב־Chromium כולל האזנה, לחישה, יציאה, שגיאת ניתוק וניסיון חוזר. build עם Webpack ו־typecheck עברו; lint ללא שגיאות עם אותן 14 אזהרות. בדיקות ה־API והעומס המלאות לעיל שייכות לסבב הקודם ולא הורצו מחדש בסבב הממוקד הזה.

ראיות: [לפני](docs/qa-20260924/evidence/monitor-races-before.log), [אחרי](docs/qa-20260924/evidence/monitor-races-after.log), [תוצאות](docs/qa-20260924/evidence/monitor-races-final.json), [דפדפן](docs/qa-20260924/evidence/monitor-ui-followup.json), [build](docs/qa-20260924/evidence/monitor-build.log). קבצים ששונו: `src/lib/dialer/monitor.ts`, `src/lib/telephony/events.ts`, `scripts/qa-provider.ts` והדוח/ראיות.

לא בוצעו שיחות חיצוניות או פריסה; בדיקת שמע אמיתי ובידוד לחישה עדיין חסומות כפי שתואר לעיל.
