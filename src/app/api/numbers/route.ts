...
import { numberConfig, telnyxNumberProvider } from "@/lib/numbers/providers";
import { lockNumberPool, numberPolicySchema } from "@/lib/numbers/selection";
import { numberOperationLimit, checkNumberConnection, syncNumbers, createNumberQuote, confirmNumberPurchase, reconcileNumberOrder, saveNumberPolicy } from "@/lib/numbers/service";
...
export const GET=withAuth(async({user,req})=>{
  ...
  const [numbers,attempts,answered,connection,orders,lists,users]=await Promise.all([
    prisma.phoneNumber.findMany({where:{businessId:user.businessId},orderBy:{createdAt:"asc"}}),
    prisma.call.groupBy({by:["phoneNumberId"],where:{businessId:user.businessId,direction:"outbound",createdAt:{gte:since}},_count:{_all:true}}),
    prisma.call.groupBy({by:["phoneNumberId"],where:{businessId:user.businessId,direction:"outbound",createdAt:{gte:since},answeredAt:{not:null}},_count:{_all:true}}),
    prisma.numberConnection.findUnique({where:{businessId:user.businessId}}),
    prisma.numberOrder.findMany({where:{businessId:user.businessId},orderBy:{createdAt:"desc"},take:100}),
    prisma.dialList.findMany({where:{businessId:user.businessId,archivedAt:null},select:{id:true,name:true,phoneNumberId:true,numberPolicy:true}}),
    prisma.user.findMany({where:{businessId:user.businessId,isActive:true},select:{id:true,fullName:true}}),
  ]);
  const cfg=numberConfig(user.businessId);
  const verified=cfg.configured&&connection?.status==="verified"&&connection.fingerprint===cfg.fingerprint&&connection.checkedAt&&Date.now()-connection.checkedAt.getTime()<86400000;
  return ok({days,since,numbers:numbers.map(n=>{const a=attempts.find(c=>c.phoneNumberId===n.id)?._count._all??0,b=answered.find(c=>c.phoneNumberId===n.id)?._count._all??0;return {...n,attempts:a,answered:b,answerRate:a?Math.round(b/a*100):null,verificationStale:!n.verifiedAt||Date.now()-n.verifiedAt.getTime()>86400000,reputationStale:!!n.reputationCheckedAt&&Date.now()-n.reputationCheckedAt.getTime()>7*86400000};}),orders,lists,users,role:user.role,
    provider:{name:"Telnyx",configured:cfg.configured,status:verified?"verified":connection?.status==="failed"?"failed":cfg.configured?"unverified":"unconfigured",checkedAt:connection?.checkedAt??null,error:connection?.error??null,purchasesEnabled:cfg.purchasesEnabled,portal:"https://portal.telnyx.com/#/voice/my-numbers"},
    reputation:{status:"unsupported",source:"Truecaller",message:"לא קיימת בחיבור הנוכחי הרשאה מאומתת ל־API מוניטין. זיהוי משתמשים אינו בדיקת ספאם. בדיקות אוטומטיות ומתוזמנות אינן זמינות.",portal:"https://business.truecaller.com/"},simulation:process.env.TELEPHONY_PROVIDER!=="telnyx"});
},{minRole:"manager"});
...
export const POST=withAuth(async({user,req})=>{
  ...
  if(["connection","sync","search","quote","purchase","reconcile"].includes(b.action))requireRole(user,"admin");
  ...
  if(b.action==="search")return ok(await telnyxNumberProvider(user.businessId).search(b.country,b.type));
